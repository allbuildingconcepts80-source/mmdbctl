...
// TestReadIPv4FromIPv6Database verifies that IPv4 lookups work on IPv6
// databases. This is a regression test: netip.AddrFromSlice on a 16-byte
// net.IP (as returned by net.ParseIP for IPv4) produces an IPv4-mapped-IPv6
// address (::ffff:x.x.x.x) which maxminddb-golang/v2 does not resolve
// through the IPv4 subtree. The fix is to call .Unmap() before lookup.
func TestReadIPv4FromIPv6Database(t *testing.T) {
